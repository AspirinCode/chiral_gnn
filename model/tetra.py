...
class TetraPermuter(nn.Module):
    ...
    def forward(self, x, n_neighbor):
        ...
        for p in self.perms[n_neighbor]:
            nei_messages_list = [self.drop(F.tanh(l(t))) for l, t in zip(self.W_bs[n_neighbor], torch.split(x[:, p, :], 1, dim=1))]
...
class ConcatTetraPermuter(nn.Module):
    ...
    def forward(self, x, n_neighbor):
        ...
        for p in self.perms[n_neighbor]:
            nei_messages += self.drop(F.relu(self.W_bs[n_neighbor](x[:, p, :].view(x.size(0), self.hidden*self.n_nums[n_neighbor]))))
        return self.mlp_out(nei_messages / 3.)
